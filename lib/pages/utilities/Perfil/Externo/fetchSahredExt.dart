...
class FetchSharedExternos extends StatefulWidget {
  final String userId;
  final String mediaUrl;
  final int initialIndex;

  const FetchSharedExternos({
    super.key,
  ...
  @override
  State createState() => _FetchSharedExternosState();
}

class _FetchSharedExternosState extends State<FetchSharedExternos>
    ...
    try {
      ...
      if (userDoc.exists) {
        ...
        List<dynamic> favoritePostIds =
            userData?['sharedPosts'] ?? []; // Lista de IDs en 'sharedPosts'

        if (favoritePostIds.isNotEmpty) {
          // Obtener los posts favoritos basados en los IDs
          await fetchSharedPosts(favoritePostIds);
        } else {
  ...
  Future<void> fetchSharedPosts(List<dynamic> favoritePostIds) async {
    try {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
            'Compartidos de ${publicaciones.isNotEmpty ? publicaciones[0]['username'] : 'Usuario'}'),
      ),
