...
import 'package:xtats001/pages/utilities/Perfil/Externo/fetchFavoritosExt.dart';
import 'package:xtats001/pages/utilities/Perfil/Externo/fetchSahredExt.dart';
import 'package:xtats001/pages/utilities/Perfil/Externo/pubusuext.dart';
...
class PerfilUsuarioScreenState extends State<PerfilUsuarioScreen>
  ...
  Future<void> getUserFavorites(String userId) async {
    try {
      // Obtener el documento del usuario por el userId
      ...
      if (userDoc.exists) {
        ...
        List<dynamic> favoritePostIds = userFavorites?['favorites'] ?? [];
  ...
  Future<void> getUserShared(String userId) async {
    try {
      // Obtener el documento del usuario por el userId
      DocumentSnapshot userDoc = await FirebaseFirestore.instance
          .collection('users')
      ...
      if (userDoc.exists) {
        ...
        List<dynamic> sharedPostIds = userShared?['sharedPosts'] ?? [];
  ...
  Future<void> fetchSharedPosts(List<dynamic> sharedPostIds) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text("Error obteniendo los posts compartidos")));
    }
  }
              ...
              return GestureDetector(
                onTap: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => PublicacionesUsuExterno(
                        userId: userData!['uid'] ?? '',
              ...
              return GestureDetector(
                onTap: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => FetchFavoritosExternos(
                        userId: userData!['uid'] ?? '',
                        mediaUrl: mediaUrl,
                        initialIndex: index,
                      ),
              ...
              return GestureDetector(
                onTap: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => FetchSharedExternos(
                        userId: userData!['uid'] ?? '',
                        mediaUrl: mediaUrl,
                        initialIndex: index,
                      ),
